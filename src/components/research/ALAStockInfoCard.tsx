    ...
    <Card className="bg-card border-border">
      <CardContent className="p-5 space-y-5">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-lg font-semibold text-foreground">{companyName || ticker}</h2>
        ...
        <div className="grid grid-cols-4 gap-3">
          <div>
            ...
            <p className="text-sm font-semibold tabular-nums">{formatCurrency(open)}</p>
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider">HIGH</p>
            <p className="text-sm font-semibold tabular-nums text-emerald-400">{formatCurrency(high)}</p>
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider">LOW</p>
            <p className="text-sm font-semibold tabular-nums text-rose-400">{formatCurrency(low)}</p>
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider">PREV CLOSE</p>
            <p className="text-sm font-semibold tabular-nums">{previousClose ? formatCurrency(previousClose) : '—'}</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="relative h-2 bg-secondary rounded-full overflow-hidden">
            ...
            />
            <div 
              className="absolute top-1/2 -translate-y-1/2 w-2.5 h-2.5 bg-white rounded-full border-2 border-primary shadow-md"
        ...
        <div className="grid grid-cols-3 gap-3 pt-2 border-t border-border">
          ...
          <div className="p-2 bg-secondary/30 rounded-md">
            ...
            <p className="text-sm font-semibold">{formatVolume(volume)}</p>
            {volume && <p className="text-[10px] text-muted-foreground">Avg: {formatVolume((volume || 0) * 0.92)}</p>}
          </div>
          <div className="p-2 bg-secondary/30 rounded-md">
            <p className="text-[10px] text-muted-foreground uppercase">Beta</p>
            <p className="text-sm font-semibold">{beta?.toFixed(2) || '—'}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3 pt-2 border-t border-border">
          <div>
            <p className="text-[10px] text-muted-foreground uppercase">P/E (TTM)</p>
            <p className="text-sm font-semibold">{peRatio?.toFixed(1) || '—'}</p>
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground uppercase">Forward P/E</p>
            <p className="text-sm font-semibold">{forwardPE?.toFixed(1) || '—'}</p>
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground uppercase">EPS (TTM)</p>
            <p className="text-sm font-semibold">{eps ? `$${eps.toFixed(2)}` : '—'}</p>
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground uppercase">Dividend</p>
            <p className="text-sm font-semibold">{dividend ? `${dividend.toFixed(2)}%` : '—'}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 pt-2 border-t border-border">
          <div className="flex flex-col">
            <p className="text-[10px] text-muted-foreground uppercase">Analyst Rating</p>
            <Badge className={cn("w-fit mt-1", ratingColors[analystRating])}>
              {analystRating}
            </Badge>
          </div>
          <div className="flex flex-col">
            <div className="flex items-center gap-1">
              <p className="text-[10px] text-muted-foreground uppercase">Price Target</p>
              <HelpCircle className="h-3 w-3 text-muted-foreground" />
            </div>
            {priceTarget && (
              <>
                <p className="text-sm font-semibold">{formatCurrency(priceTarget)}</p>
                <p className={cn(
                  "text-[10px]",
                  priceTarget > price ? "text-emerald-400" : "text-rose-400"
                )}>
                  ({priceTarget > price ? '+' : ''}{(((priceTarget - price) / price) * 100).toFixed(1)}%)
                </p>
              </>
            )}
          </div>
          <div className="flex flex-col">
            ...
            <p className="text-sm font-semibold">{nextEarnings || '—'}</p>
          </div>
        </div>
      </CardContent>
    </Card>
