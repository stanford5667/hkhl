...
function studyMovingAverageAnalysis(bars: PriceBar[], params?: { periods?: number[] }) { const periods = params?.periods || [20, 50, 200]; const closes = bars.map(b => b.close); const results: Record<string, any> = {}; for (const period of periods) { if (bars.length < period) continue; const sma = calculateSMA(closes, period); const ema = calculateEMA(closes, period); let aboveSMA = 0, aboveEMA = 0; for (let i = period - 1; i < bars.length; i++) { if (closes[i] > sma[i]) aboveSMA++; if (closes[i] > ema[i]) aboveEMA++; } const validCount = bars.length - period + 1; const currentAboveSMA = closes[closes.length - 1] > sma[sma.length - 1]; const currentAboveEMA = closes[closes.length - 1] > ema[ema.length - 1]; const distFromSMA = ((closes[closes.length - 1] - sma[sma.length - 1]) / sma[sma.length - 1]) * 100; const distFromEMA = ((closes[closes.length - 1] - ema[ema.length - 1]) / ema[ema.length - 1]) * 100; results[`ma${period}`] = { sma: sma[sma.length - 1], ema: ema[ema.length - 1], pctAboveSMA: (aboveSMA / validCount) * 100, pctAboveEMA: (aboveEMA / validCount) * 100, currentAboveSMA, currentAboveEMA, distFromSMA, distFromEMA }; } if (periods.includes(50) && periods.includes(200) && bars.length >= 200) { const sma50 = calculateSMA(closes, 50); const sma200 = calculateSMA(closes, 200); const crosses: { date: string; type: 'golden' | 'death' }[] = []; for (let i = 200; i < bars.length; i++) { const prev50Above200 = sma50[i - 1] > sma200[i - 1]; const curr50Above200 = sma50[i] > sma200[i]; if (!prev50Above200 && curr50Above200) crosses.push({ date: bars[i].date, type: 'golden' }); else if (prev50Above200 && !curr50Above200) crosses.push({ date: bars[i].date, type: 'death' }); } results.crosses = crosses.slice(-5); results.currentTrend = sma50[sma50.length - 1] > sma200[sma200.length - 1] ? 'bullish' : 'bearish'; } return { type: 'moving_average', ...results }; }

function studyVolumeAnalysis(bars: PriceBar[]) { const volumes = bars.map(b => b.volume); const avgVolume = volumes.reduce((a, b) => a + b, 0) / volumes.length; const upDaysVolume: number[] = []; const downDaysVolume: number[] = []; for (let i = 1; i < bars.length; i++) { if (bars[i].close > bars[i - 1].close) upDaysVolume.push(bars[i].volume); else if (bars[i].close < bars[i - 1].close) downDaysVolume.push(bars[i].volume); } const avgUpVolume = upDaysVolume.length > 0 ? upDaysVolume.reduce((a, b) => a + b, 0) / upDaysVolume.length : 0; const avgDownVolume = downDaysVolume.length > 0 ? downDaysVolume.reduce((a, b) => a + b, 0) / downDaysVolume.length : 0; const highVolThreshold = avgVolume * 1.5; const highVolDays = bars.filter(b => b.volume > highVolThreshold); const highVolReturns: number[] = []; for (const hvd of highVolDays) { const idx = bars.findIndex(b => b.date === hvd.date); if (idx > 0) highVolReturns.push(((hvd.close - bars[idx - 1].close) / bars[idx - 1].close) * 100); } const recentAvgVol = volumes.slice(-20).reduce((a, b) => a + b, 0) / 20; const volumeTrend = ((recentAvgVol - avgVolume) / avgVolume) * 100; return { type: 'volume', avgVolume, currentVolume: volumes[volumes.length - 1], volumeRatio: volumes[volumes.length - 1] / avgVolume, upDayAvgVolume: avgUpVolume, downDayAvgVolume: avgDownVolume, volumeBias: avgUpVolume > avgDownVolume ? 'accumulation' : 'distribution', highVolumeDays: { count: highVolDays.length, avgReturn: highVolReturns.length > 0 ? highVolReturns.reduce((a, b) => a + b, 0) / highVolReturns.length : 0, hitRate: highVolReturns.length > 0 ? (highVolReturns.filter(r => r > 0).length / highVolReturns.length) * 100 : 0 }, volumeTrend, percentiles: { p25: percentile(volumes, 25), p50: percentile(volumes, 50), p75: percentile(volumes, 75), p90: percentile(volumes, 90) } }; }

function studyRSIAnalysis(bars: PriceBar[]) { const rsi = calculateRSI(bars, 14); const validRSI = rsi.filter(x => !isNaN(x)); const overbought = validRSI.filter(r => r > 70).length; const oversold = validRSI.filter(r => r < 30).length; const afterOverbought: number[] = []; const afterOversold: number[] = []; for (let i = 14; i < bars.length - 5; i++) { if (rsi[i - 14] > 70) afterOverbought.push(((bars[i + 5].close - bars[i].close) / bars[i].close) * 100); else if (rsi[i - 14] < 30) afterOversold.push(((bars[i + 5].close - bars[i].close) / bars[i].close) * 100); } const rsiRanges = [{ range: '0-20', count: validRSI.filter(r => r >= 0 && r < 20).length }, { range: '20-30', count: validRSI.filter(r => r >= 20 && r < 30).length }, { range: '30-50', count: validRSI.filter(r => r >= 30 && r < 50).length }, { range: '50-70', count: validRSI.filter(r => r >= 50 && r < 70).length }, { range: '70-80', count: validRSI.filter(r => r >= 70 && r < 80).length }, { range: '80-100', count: validRSI.filter(r => r >= 80 && r <= 100).length }]; return { type: 'rsi', current: validRSI[validRSI.length - 1], avg: validRSI.reduce((a, b) => a + b, 0) / validRSI.length, overboughtPct: (overbought / validRSI.length) * 100, oversoldPct: (oversold / validRSI.length) * 100, afterOverbought: { count: afterOverbought.length, avgReturn: afterOverbought.length > 0 ? afterOverbought.reduce((a, b) => a + b, 0) / afterOverbought.length : 0, hitRate: afterOverbought.length > 0 ? (afterOverbought.filter(r => r > 0).length / afterOverbought.length) * 100 : 0 }, afterOversold: { count: afterOversold.length, avgReturn: afterOversold.length > 0 ? afterOversold.reduce((a, b) => a + b, 0) / afterOversold.length : 0, hitRate: afterOversold.length > 0 ? (afterOversold.filter(r => r > 0).length / afterOversold.length) * 100 : 0 }, distribution: rsiRanges }; }

function studyMeanReversion(bars: PriceBar[]) { const returns: number[] = []; for (let i = 1; i < bars.length; i++) returns.push(((bars[i].close - bars[i - 1].close) / bars[i - 1].close) * 100); const mean = returns.reduce((a, b) => a + b, 0) / returns.length; const stdDev = Math.sqrt(returns.reduce((sum, r) => sum + Math.pow(r - mean, 2), 0) / returns.length); const afterLargeUp: number[] = []; const afterLargeDown: number[] = []; for (let i = 0; i < returns.length - 1; i++) { if (returns[i] > mean + 2 * stdDev) afterLargeUp.push(returns[i + 1]); else if (returns[i] < mean - 2 * stdDev) afterLargeDown.push(returns[i + 1]); } let autocorr = 0; for (let i = 1; i < returns.length; i++) autocorr += (returns[i] - mean) * (returns[i - 1] - mean); autocorr /= (returns.length - 1) * stdDev * stdDev; return { type: 'mean_reversion', autocorrelation: autocorr, regime: autocorr < -0.1 ? 'mean_reverting' : autocorr > 0.1 ? 'trending' : 'random', afterLargeUp: { count: afterLargeUp.length, avgNextDayReturn: afterLargeUp.length > 0 ? afterLargeUp.reduce((a, b) => a + b, 0) / afterLargeUp.length : 0, reversalRate: afterLargeUp.length > 0 ? (afterLargeUp.filter(r => r < 0).length / afterLargeUp.length) * 100 : 0 }, afterLargeDown: { count: afterLargeDown.length, avgNextDayReturn: afterLargeDown.length > 0 ? afterLargeDown.reduce((a, b) => a + b, 0) / afterLargeDown.length : 0, reversalRate: afterLargeDown.length > 0 ? (afterLargeDown.filter(r => r > 0).length / afterLargeDown.length) * 100 : 0 } }; }
...
function studyHighLowAnalysis(bars: PriceBar[]) { let newHighs = 0, newLows = 0; const lookback = 20; const newHighDates: string[] = []; const newLowDates: string[] = []; const afterNewHigh: number[] = []; const afterNewLow: number[] = []; for (let i = lookback; i < bars.length; i++) { const recentHighs = bars.slice(i - lookback, i).map(b => b.high); const recentLows = bars.slice(i - lookback, i).map(b => b.low); if (bars[i].high > Math.max(...recentHighs)) { newHighs++; newHighDates.push(bars[i].date); if (i < bars.length - 5) afterNewHigh.push(((bars[i + 5].close - bars[i].close) / bars[i].close) * 100); } if (bars[i].low < Math.min(...recentLows)) { newLows++; newLowDates.push(bars[i].date); if (i < bars.length - 5) afterNewLow.push(((bars[i + 5].close - bars[i].close) / bars[i].close) * 100); } } const yearBars = bars.slice(-252); const yearHigh = Math.max(...yearBars.map(b => b.high)); const yearLow = Math.min(...yearBars.map(b => b.low)); const currentPrice = bars[bars.length - 1].close; return { type: 'high_low', newHighs: { count: newHighs, rate: (newHighs / (bars.length - lookback)) * 100, avg5DayReturn: afterNewHigh.length > 0 ? afterNewHigh.reduce((a, b) => a + b, 0) / afterNewHigh.length : 0, hitRate: afterNewHigh.length > 0 ? (afterNewHigh.filter(r => r > 0).length / afterNewHigh.length) * 100 : 0 }, newLows: { count: newLows, rate: (newLows / (bars.length - lookback)) * 100, avg5DayReturn: afterNewLow.length > 0 ? afterNewLow.reduce((a, b) => a + b, 0) / afterNewLow.length : 0, hitRate: afterNewLow.length > 0 ? (afterNewLow.filter(r => r > 0).length / afterNewLow.length) * 100 : 0 }, yearHigh, yearLow, distFromHigh: ((currentPrice - yearHigh) / yearHigh) * 100, distFromLow: ((currentPrice - yearLow) / yearLow) * 100, recentNewHighs: newHighDates.slice(-5), recentNewLows: newLowDates.slice(-5) }; }

function studyTrendStrength(bars: PriceBar[]) { const closes = bars.map(b => b.close); const sma20 = calculateSMA(closes, 20); const sma50 = calculateSMA(closes, 50); const sma200 = bars.length >= 200 ? calculateSMA(closes, 200) : []; let score = 0; const curr = bars.length - 1; if (!isNaN(sma20[curr]) && closes[curr] > sma20[curr]) score += 1; if (!isNaN(sma50[curr]) && closes[curr] > sma50[curr]) score += 1; if (sma200.length > 0 && !isNaN(sma200[curr]) && closes[curr] > sma200[curr]) score += 1; if (!isNaN(sma20[curr]) && !isNaN(sma50[curr]) && sma20[curr] > sma50[curr]) score += 1; if (sma200.length > 0 && !isNaN(sma50[curr]) && !isNaN(sma200[curr]) && sma50[curr] > sma200[curr]) score += 1; const recentBars = bars.slice(-20); let higherHighs = 0, higherLows = 0; for (let i = 1; i < recentBars.length; i++) { if (recentBars[i].high > recentBars[i - 1].high) higherHighs++; if (recentBars[i].low > recentBars[i - 1].low) higherLows++; } const maxScore = sma200.length > 0 ? 5 : 3; return { type: 'trend_strength', trendScore: score, maxScore, trendDirection: score >= maxScore - 1 ? 'strong_up' : score >= maxScore / 2 ? 'up' : score <= 1 ? 'strong_down' : score <= maxScore / 2 ? 'down' : 'neutral', aboveSMA20: !isNaN(sma20[curr]) && closes[curr] > sma20[curr], aboveSMA50: !isNaN(sma50[curr]) && closes[curr] > sma50[curr], aboveSMA200: sma200.length > 0 && !isNaN(sma200[curr]) && closes[curr] > sma200[curr], sma20AboveSMA50: !isNaN(sma20[curr]) && !isNaN(sma50[curr]) && sma20[curr] > sma50[curr], sma50AboveSMA200: sma200.length > 0 && !isNaN(sma50[curr]) && !isNaN(sma200[curr]) && sma50[curr] > sma200[curr], higherHighsRate: (higherHighs / (recentBars.length - 1)) * 100, higherLowsRate: (higherLows / (recentBars.length - 1)) * 100 }; }
...
serve(async (req) => {
  ...
  try {
    ...
    switch (studyType) {
      ...
      case 'volume_analysis': result = studyVolumeAnalysis(bars); break;
      case 'rsi_analysis': result = studyRSIAnalysis(bars); break;
      case 'mean_reversion': result = studyMeanReversion(bars); break;
      case 'range_analysis': result = studyRangeAnalysis(bars); break;
      case 'high_low_analysis': result = studyHighLowAnalysis(bars); break;
      case 'trend_strength': result = studyTrendStrength(bars); break;
