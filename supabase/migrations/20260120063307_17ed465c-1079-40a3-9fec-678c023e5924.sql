-- 1) Data quality flag for study_probability_scores
ALTER TABLE public.study_probability_scores
ADD COLUMN IF NOT EXISTS is_valid BOOLEAN NOT NULL DEFAULT true;

COMMENT ON COLUMN public.study_probability_scores.is_valid IS
'Valid + complete calculation (sample_size >= 3 and required metrics present)';

UPDATE public.study_probability_scores
SET is_valid = true
WHERE sample_size >= 3
...
WHERE sample_size < 3
   ...
   OR win_rate IS NULL
   OR expected_return IS NULL;

CREATE INDEX IF NOT EXISTS idx_sps_valid_probability
ON public.study_probability_scores (is_valid, probability_score DESC)
WHERE is_valid = true;

-- 2) RPC function used by the cross-study screener
-- NOTE: Parameter is named *_filter to avoid a PL/pgSQL name collision with the output column.
